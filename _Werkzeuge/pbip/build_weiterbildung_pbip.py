...
03-Fall-Weiterbildung/.

Ablauf (siehe _Werkzeuge/pbip/README.md):
...
Aufruf aus dem Repo-Root:
  python3 _Werkzeuge/pbip/build_weiterbildung_pbip.py   # schreibt 03-Fall-Weiterbildung/PowerBI-Loesung/
"""
import json
import shutil
import subprocess
...
OUT = ROOT / "03-Fall-Weiterbildung" / "PowerBI-Loesung"
SM = OUT / f"{NAME}.SemanticModel"
RP = OUT / f"{NAME}.Report"
SPEC = json.loads((ROOT / "03-Fall-Weiterbildung/Mockup/mockup-spec.json").read_text(encoding="utf-8"))
DATEN_URL = ("https://raw.githubusercontent.com/Losveratos/Power-Starter-24-25-September/main/"
             "03-Fall-Weiterbildung/Daten/")
...
let
    ...
    Quelle = Csv.Document({W("1-Rohdaten/weiterbildung_buchungen_2025.csv")}, [Delimiter = ";", Columns = 11, Encoding = 1252, QuoteStyle = QuoteStyle.None]),
...
M_BUDGET = f"""
let
    Quelle = Excel.Workbook({W("3-Ziele/ziele_2025.xlsx")}, null, true),
...
TABLES = [
    ...
    ("dim_mitarbeitende", "HR-Stamm, eine Zeile je Person (pseudonymisiert)", m_csv(
        "2-Anreicherung/dim_mitarbeitende.csv",
    ...
    ("dim_bereich", "Organisationsbereiche", m_csv(
        "2-Anreicherung/dim_bereich.csv",
...
def build_visuals():
    ...
    vs.append(textbox("fussleiste", {"x": f["x"] + 16, "y": f["y"], "w": f["w"] - 32, "h": f["h"]}, 1, [
        ("Quelle: LMS-Export + HR-Stamm · Stand: Monatsende · Kennzahl-Definitionen: 03-Fall-Weiterbildung/Loesungen/DAX/measures.dax",
         SEGOE, "8pt", GREY)]))
