...
Aufruf aus dem Repo-Root:
  ...
  PLAYWRIGHT_CHROMIUM=/pfad/zu/chrome python3 tools/build_downloads.py
"""
import os
import re
import tempfile
...
def main():
    ...
    case = ROOT / "cases/weiterbildungs-monitoring"
    pdf = {
    ...
    render_pdfs([(ROOT / "agenda.md", pdf["agenda"]), (ROOT / "handout.md", pdf["handout"]),
                 (ROOT / "uebungen/tag1.md", pdf["tag1"]), (ROOT / "uebungen/tag2.md", pdf["tag2"]),
                 (case / "README.md", pdf["fall"]), (ROOT / "pbip/Weiterbildungs-Monitoring/README.md", pdf["pbip"])])
    ...
    write_zip("Starter-Training-Tag1.zip", [
        ...
        ("Daten/verkaeufe_2025_roh.csv", ROOT / "daten/tag1/verkaeufe_2025_roh.csv"),
        ("Loesungen/Tag1_PowerQuery_Aufraeumen.txt", ROOT / "snippets/powerquery/tag1_verkaeufe_bereinigen.pq"),
    ])
    write_zip("Starter-Training-Tag2.zip", [
        ...
        *[(f"Daten/{f.name}", f) for f in sorted((ROOT / "daten/tag2").iterdir())],
        ("Loesungen/Tag2_PowerQuery_CSV_laden.txt", ROOT / "snippets/powerquery/tag2_csv_von_github.pq"),
        ("Loesungen/Tag2_PowerQuery_Kalender.txt", ROOT / "snippets/powerquery/kalender.pq"),
        ("Loesungen/Tag2_PowerQuery_Plan_entpivotieren.txt", ROOT / "snippets/powerquery/plan_entpivotieren.pq"),
        ("Loesungen/Tag2_DAX_Measures.txt", ROOT / "snippets/dax/measures_tag2.dax"),
        ("Loesungen/Tag2_DAX_Kalender.txt", ROOT / "snippets/dax/kalender_dax.dax"),
    ])
    write_zip("Fall-Weiterbildungs-Monitoring.zip", [
        ...
        *[(f"Loesungen/{f.stem}.txt", f) for f in sorted((case / "skripte").rglob("*")) if f.is_file()],
    ])
...
"""),
        ...
        *tree(ROOT / "pbip/Weiterbildungs-Monitoring", "Weiterbildungs-Monitoring",
              skip=(".pbi",)),
    ])
